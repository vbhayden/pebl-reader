...
echo "function consoleLog(...x) { if (DEBUGGING) { console.log(...x); } }; function consoleError(...x) { if (DEBUGGING) { console.error(...x); } }" | cat - readium-js-viewer_CLOUDAPP-WORKER.js > readium-js-viewer_CLOUDAPP-WORKER.js2
